          ...
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-2">
              ...
              <button
                ...
                disabled={refreshing}
                title="Buscar plantillas nuevas directamente en GitHub, sin esperar una actualización de la app"
                className="flex flex-none items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium text-text-dim hover:text-text"
              >
                <RefreshCw size={11} className={refreshing ? 'animate-spin' : ''} /> {refreshing ? 'Buscando…' : 'Actualizar'}
              </button>
            </div>
              ...
              {entries.map((e) => (
                <button key={e.fileName} className="w-full text-left" onClick={() => setTemplate(e.template)}>
                  <Card className="p-3.5 hover:border-primary/40">
                    ...
                    {e.template.autorPublico && <div className="mt-1 text-[11px] text-text-dim">por {e.template.autorPublico}</div>}
                  </Card>
                </button>
              ))}
              {entries.length === 0 && <div className="py-6 text-center text-xs text-text-dim">No hay plantillas incluidas.</div>}
            </div>

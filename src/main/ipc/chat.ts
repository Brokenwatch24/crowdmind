...
export function registerChatHandlers(): void {
  ...
  ipcMain.handle(
    IPC.chatSend,
    async (_e, input: { personaId: string; workspaceId: string; mensaje: string; provider: ProviderId; model?: string }) => {
      ...
      const respuestaTexto = await getPersonaChatReply(call, persona, historia, input.mensaje)
